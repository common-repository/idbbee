...
function idbbee_embed_shortcode($atts, $content = null)
{
	...
	$validAttrs = ["width", "src", "height", "marginheight", "marginwidth", "border", "frameborder", "align", "name", "id", "class"];
	...
	foreach ($atts as $attr => $value) {
		if(!in_array($attr,$validAttrs))continue;
		if ($value != '') {
			$dbbeecode .= ' ' . esc_attr($attr) . '="' . esc_attr($value) . '"';
		} else {
			$dbbeecode .= ' ' . esc_attr($attr);
		}
	}
	$dbbeecode .= '></iframe>' . "\n";
	return $dbbeecode;
